...
@pytest.mark.parametrize("status_code,response_data,expected_result", [
    ...
    (200, {
        'status': ['active'],
        'events': [
            {'eventDate': '2020-01-01T00:00:00Z', 'eventAction': 'registration'},
            {'eventDate': '2025-01-01T00:00:00Z', 'eventAction': 'expiration'}
        ]
    }, {
        'domain': 'test.com',
        'status': 'REGISTERED',
...
def test_check_domain_connection_error():
    """Test domain checking with connection error"""
    with patch('requests.get') as mock_get:
